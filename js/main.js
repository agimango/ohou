$(document).ready(function() {
    ...
    if($(window).width() >= 1280) {
      ...
      $('nav > ul > li > a').click(function() {
        // a태그의 다음 객체인 ul.sub가 slideTogle됨
    ...
    function minimize_header() {
      ...
      let $header = $("header");
      //did_scroll변수를 선언하고 null(값 없음)으로 초기화
      let did_scroll = null;
    ...
    const swiper = new Swiper('.main', { 
      ...
      $(window).scroll(function() {
        ...
        $('.scrollSet').each(function() {
          ...
          // 만약 secTop이 winTop보다 작고 secBottom이 winTop보다 크면 section안의 자식객체에 active 추가
      ...
      $('.family_site > a').click(function(e) {
        // a태그의 링크 기능을 막는 기능
        e.preventDefault();
        // 만약 sw변수의 값이 0이면 .family_list나타나고, sw변수의 값을 1로 변경
        if(sw==0) {
          sw=1;
